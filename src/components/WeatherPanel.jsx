...
function WeatherPanel() {
  ...
  useEffect(() => {
    if (
      ...
      isTimeToUpdate(
        new Date(JSON.parse(localStorage.getItem("weather")).received)
      )
    ) {
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=Baku&units=metric&appid=${apiKeys.weather}`
      )
        .then((res) => res.json())
        .then((data) => {
        ...
        <>
          ...
          <div className="weather-body">
            <img
              ...
              src={`/images/weather/${weatherCache.weather[0].icon}.png`}
            />
            ...
            <div className="weather-details">
              <div className="weather-details-item">
                ...
                <span>{`${weatherCache.wind.speed}`} km/h</span>
              </div>
              <div className="weather-details-item">
                <i className="ri-drop-line"></i>{" "}
                <span>{`${weatherCache.main.humidity}`}%</span>
              </div>
              <div className="weather-details-item">
                <i className="ri-sun-line"></i>{" "}
                <span>{`${Math.floor(sunHours)}`}h</span>
              </div>
              {weatherCache.rain && (
                <div className="weather-details-item">
                  <i className="ri-rainy-line"></i>{" "}
                  {parseInt(weatherCache.rain["1h"] * 100)}%
                </div>
              )}
            </div>
            <div className="weather-big">
              {Math.floor(weatherCache.main.temp)}°
            </div>
            {/* <img className="weather-image" src={`http://openweathermap.org/img/wn/${weatherCache.weather[0].icon}@4x.png`} /> */}
          </div>
          <div className="weather-updated">
            last updated{" "}
            {getFormattedDateAndTime(new Date(weatherCache.received))}
          </div>
        </>
      );
    }
  ...
  return <div className="weather-container">{getWeatherHtml()}</div>;
}
